...
#include <vector>
#include <array>
//#include <cstdlib> needed when running system("pause");
#include <time.h>
#include <iomanip>
...
void triple2(array<int, 10> &arr);
void triple3(vector<int> *v);
void displayArray(array<int, 10> arr);
...
void triple3(vector<int> *v){
    for(int i = 0; i < 10; ++i){
    (*v)[i] *= 3;} //There is a parenthesis around (*v) because of operator precedence
} 
void displayArray(array<int, 10> arr){
    for(int i = 0; i < 10; ++i)
